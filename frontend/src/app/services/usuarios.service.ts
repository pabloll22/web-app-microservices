...
export class UsuariosService {
  ...
  doLogin(login: Login): Observable<UsuarioSesion> {
    ...
    let roles = [];
    obj.usuario.administrador?roles.push({rol: Rol.ADMINISTRADOR}):undefined;
    obj.usuario.gerente?roles.push({rol: Rol.GERENTE}):undefined;
      return {
        id: obj.usuario.id,
  ...
  agregarCentroGerente(centro: Centro, usuario: Usuario): Observable<Usuario>{
    return this.backend.agregarCentroGerente(centro, usuario);
  }

  desagregarCentroGerente(centro: Centro, usuario: Usuario): Observable<Usuario>{
    return this.backend.desagregarCentroGerente(centro, usuario);
  }
